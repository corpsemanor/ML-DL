...
class KNN:
    ...
    def predict_labels_binary(self, dists):
        ...
        for i in range(num_test):
            # Находим k ближайших тренировочных образцов
            nearest = np.argsort(dists[i])[:self.k]
            
            # Считаем количество каждого класса
            positive_count = np.count_nonzero(self.train_y[nearest])
    ...
    def predict_labels_multiclass(self, dists):
        ...
        for i in range(num_test):
            ...
            nearest = self.train_y[np.argsort(dists[i])[:self.k]].tolist()
            pred[i] = max(nearest, key=nearest.count)
        return pred
