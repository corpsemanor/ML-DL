...
class ConvolutionalLayer:
    ...
    def forward(self, X):
        ...
        for y in range(out_height):
            for x in range(out_width):
                X_after_kernel = X[:,y:y+self.filter_size,x:x+self.filter_size, :]
    ...
    def backward(self, d_out):
        ...
        for y in range(out_height):
            for x in range(out_width):
                point = d_out[:,y,x,:]
                X_after_kernel = self.X[:,y:y+self.filter_size,x:x+self.filter_size, :]
                X_after_kernel_reshaped = X_after_kernel.reshape((batch_size, -1))

                d_W = np.dot(X_after_kernel_reshaped.T, point)
                d_W = d_W.reshape((filter_size, filter_size, self.in_channels, self.out_channels))
                ...
                self.W.grad += d_W
                self.B.grad += d_B
                d_X[:,y:y+self.filter_size,x:x+self.filter_size,:] += d_X_before_kernel
